...
class ReleaseCode():
    ...
    def increment_upload(self):
        ...
        for dirpath, dirname, filenames in os.walk(top_path):
            remote_path = os.path.join(self.server_info['remote_path'], self.server_info['release_version'],dirpath[len(top_path)+1:])
            self.public_upload(dirpath, dirname, filenames, remote_path)
    ...
    def public_upload(self, dirpath, dirname, filenames, remote_path):
        try:
            sftp = self.connection.open_sftp()
            try:
                sftp.listdir(remote_path)                   # Test if remote exists
            except IOError:
                dir_path, base_name = os.path.split(remote_path.rstrip('/'))
                try:
                    sftp.mkdir(dir_path)                    # Create rmote folder
                except IOError:
                    print "Assume target folder %s exists." % dir_path
                try:
                    sftp.mkdir("%s/%s" % (dir_path, base_name))
                except IOError:
                    print "Assume %s/%s folder already exists." % (dir_path, base_name)
